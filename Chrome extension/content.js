...
function takefullpage(){
  const screenshotTarget = document.body;
  return html2canvas(screenshotTarget).then((canvas) => {
    var base64image = canvas.toDataURL("image/png");
    console.log(base64image);
    return base64image; // return the base64 image data
  });
}
...
async function captureAndExtract() {
  const screenshotPromise = takefullpage();
  const extractedText = extractText();
  const currentUrl = document.location.href;
  
  const screenshot = await screenshotPromise; // Wait for the screenshot to be captured
  console.log('body:', extractedText);
  console.log('screenshot:', screenshot);
  chrome.runtime.sendMessage({ lines: extractedText, page: screenshot , pageURL: currentUrl });
}
